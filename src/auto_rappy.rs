...
fn check_qte_appear(capture: &DxgiCapture, tx: &Sender<String>) -> bool {
    if cfg!(debug_assertions) {
        // show_image(&game_shot);
    ...
    if max_val > 0.99 {
        ...
        tx.send(format!("qte name:{}, sim: {:.6}", png_name, max_val))
            .unwrap();
...
fn wait_for_key_ready(capture: &DxgiCapture, bet_coin_is_one: &mut bool, tx: &Sender<String>) {
...
fn process_rappy_qte(capture: &DxgiCapture, burst: &mut bool, tx: &Sender<String>) {
    info!(
    ...
    ) || *burst
    {
        ...
        while !check_qte_appear(capture, tx) {
            continue;
        }
        ...
        Some(hwnd) => {
            ...
            loop {
                if WindowsKeyboard::state() {
                    // 等待按下回车
                    wait_for_key_ready(&capture, &mut bet_coin_is_one, tx);
                    ...
                    process_rappy_qte(&capture, &mut burst, tx);
                    if check_game_shot(
                        &capture,
                        &CapturePos::KEY_READY,
                        &TemplateImg::KEY_READY,
